...
When an error is returned, the `"error"` key contains a machine-readable code. If the `"error"` is, for example, an `"interaction_required"`, you might prompt the user to provide additional information to complete the authentication process. If the `"error"` is `"invalid_grant"`, you might prompt the user to reenter their credentials. The following snippet is an example of error handling in MSAL for Python.
...
In MSAL for Python, exceptions are rare because most errors are handled by returning an error value. The `ValueError` exception is only thrown when there's an issue with how you're attempting to use the library, such as when API parameters are malformed.

## Conditional Access and claims challenges

When getting tokens silently, your application might receive errors when a [Conditional Access claims challenge](/azure/active-directory/develop/v2-conditional-access-dev-guide) such as MFA policy is required by an API you're trying to access.
...
You might customize this behavior by following the [`http_client` customization instructions](xref:msal.application.ConfidentialClientApplication).
...
MSAL Python 1.16+ makes it easy for you to retry an authentication request on-demand (for example, whenever the end-user clicks the sign-in button again), MSAL Python 1.16+ would automatically throttle those retry attempts by returning same error response from an HTTP cache, and only sending out a real HTTP call when that call is attempted after the specified period.

By default, this throttle mechanism works by saving throttle information into a built-in in-memory HTTP cache.
You might provide your own `dict`-like object as the HTTP cache, which you can control how to persist its content.
